...
def load_data(file_path):
    ...
    df['fpc'] = df['interface_id'].apply(extract_fpc)
...
def get_normalized_name_counts(df, label=""):
    unique_names = df['interface_id'].dropna().unique()
...
def get_unique_interface_id_counts_per_field(df, label):
    """
    Returns a DataFrame with count of unique interface names per field.
    """
    counts = df.groupby('field')['interface_id'].nunique().reset_index()
...
def main(steady_path, reboot_path, fpcs=None):
    ...
    if fpcs is not None:
        ...
        output_file = "merged_fpc_qos_comparison.xlsx"
    else:
        output_file = "merged_all_qos_comparison.xlsx"
    ...
    group_cols_nonnum = ['path', 'interface_id', 'field', 'value', 'component', 'component_id']
    ...
    group_cols_num = ['path', 'interface_id', 'field', 'component', 'component_id']
    ...
    group_cols_ordered = ['path', 'interface_id', 'field', 'value', 'component', 'component_id']
    ...
    steady_field_interface_id_counts = get_unique_interface_id_counts_per_field(steady_df, steady_label)
    reboot_field_interface_id_counts = get_unique_interface_id_counts_per_field(reboot_df, reboot_label)

    merged_field_interface_id_counts = pd.merge(
        steady_field_interface_id_counts,
        reboot_field_interface_id_counts,
        on='field',
        how='outer'
    ).fillna(0)

    merged_field_interface_id_counts[f'{steady_label}_name_count'] = merged_field_interface_id_counts[f'{steady_label}_name_count'].astype(int)
    merged_field_interface_id_counts[f'{reboot_label}_name_count'] = merged_field_interface_id_counts[f'{reboot_label}_name_count'].astype(int)
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        merged_field_interface_id_counts.to_excel(writer, sheet_name='FieldInterfaceIDCounts', index=False)
