...
def extract_fpc(name):
    ...
    if match:
        ...
        return "" # Almost all are AE interfaces and hence not wise to use et- to filter based on FPC
    return None
...
def get_normalized_filter_counts(df, label=""):
    if 'Custom1' not in df.columns:
        return pd.DataFrame(columns=['normalized_filter', f'{label}_count'])
    unique_names = df['Custom1'].dropna().unique()
    normalized_names = [normalize_name(name) for name in unique_names]

    name_counts = pd.Series(normalized_names).value_counts().reset_index()
    name_counts.columns = ['normalized_filter', f'{label}_count']
    return name_counts
...
def main(steady_path, reboot_path, fpcs=None):
    ...
    if fpcs is not None:
        ...
        output_file = "summery_all_junos_comparison.xlsx"
    else:
        output_file = "summery_all_junos_comparison.xlsx"
    ...
    group_cols_nonnum = ['path', 'field', 'value', 'component', 'component_id', 'name']
    if 'Custom1' in steady_df.columns and 'Custom1' in reboot_df.columns:
        group_cols_nonnum.append('Custom1')
    ...
    group_cols_num = ['path', 'field', 'component', 'component_id', 'name', 'value']
    if 'Custom1' in steady_df.columns and 'Custom1' in reboot_df.columns:
        group_cols_num.append('Custom1')
    ...
    if 'Custom1' in steady_df.columns and 'Custom1' in reboot_df.columns:
        group_cols_ordered = ['path', 'name', 'field', 'value', 'component', 'component_id', 'Custom1']
    ...
    steady_filter_counts = get_normalized_filter_counts(steady_df, steady_label)
    reboot_filter_counts = get_normalized_filter_counts(reboot_df, reboot_label)

    filter_counts_merged = pd.merge(
        steady_filter_counts,
        reboot_filter_counts,
        on='normalized_filter',
        how='outer'
    ).fillna(0)

    filter_counts_merged[f'{steady_label}_count'] = filter_counts_merged[f'{steady_label}_count'].astype(int)
    filter_counts_merged[f'{reboot_label}_count'] = filter_counts_merged[f'{reboot_label}_count'].astype(int)
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        filter_counts_merged.to_excel(writer, sheet_name='CustomFilterCounts', index=False)
...
if __name__ == "__main__":
    ...
    main(args.steady_csv, args.reboot_csv)
